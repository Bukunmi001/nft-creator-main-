...
module overmind::NonFungibleToken {
    ...
    */
    ...
    // Initialize the minter cap object
    fun init(ctx: &mut TxContext) {
        // Initialize the minter cap object and transfer it to the deployer of the module
        let minter_cap = MinterCap {
            id: ctx.get_object_id(),
            sales: Balance::zero(),
        };
        ctx.create_object(&minter_cap);
    }
    ...
    public fun mint_nft(
        ...
        // Ensure payment is sufficient for minting
        if payment_coin.amount < 1 {
        ...
        let new_nft = NonFungibleToken {
            id: ctx.get_object_id(),
            name: String::from_utf8(nft_name).unwrap(),
            description: String::from_utf8(nft_description).unwrap(),
            image: Url::from_bytes(nft_image).unwrap(),
        };

        // Transfer the NFT to the recipient
        transfer::transfer_object(recipient, &new_nft, ctx);

        // Update the sales balance in the MinterCap object
        minter_cap.sales += payment_coin.amount - 1;

        // Return change to the sender
        payment_coin.amount = 1;
    }

    // Combine two NFTs into a new NFT
    public fun combine_nfts(
        ...
        let new_nft = NonFungibleToken {
            id: ctx.get_object_id(),
            name: format!("{} + {}", nft1.name, nft2.name),
            description: format!("Combined NFT of {} and {}", nft1.name, nft2.name),
            image: Url::from_bytes(new_image_url).unwrap(),
        };

        // Delete the two input NFTs
        ctx.delete_object(&nft1);
        ctx.delete_object(&nft2);

        // Transfer the new NFT to the sender
        transfer::transfer_object(ctx.get_caller(), &new_nft, ctx);

        // Return the new NFT object
        new_nft
    }

    // Withdraw sales balance from the MinterCap object
    public fun withdraw_sales(
        minter_cap: &mut MinterCap,
        ctx: &mut TxContext,
    ): Coin<SUI> {
        // Transfer the sales balance to the owner
        let withdrawn_amount = minter_cap.sales;
        minter_cap.sales = 0;
        transfer::transfer_coin(ctx.get_caller(), withdrawn_amount, ctx);
    ...
    // Delete an NFT object
    public fun burn_nft(nft: NonFungibleToken) {
        // Delete the NFT object
        ctx.delete_object(&nft);

        // Emit the NonFungibleTokenDeleted event
